import { searchVideoForm, boxVideoList, boxPlayList, cleanToSearchButton, searchWordDiv, removeSearchWord, searchWordBox } from "./../variables.js";
import { UIcleanToList, UIsearchWordPrint, UIresetForm } from './../UIController/uiVideoListController.js'
...
const searchVideoItem = (e) => {
    ...
    searchVideoWord.includes(videoname) || videoname === '' ? null : searchVideoWord = [...searchVideoWord, videoname]
...
export function editToListSearchVideo(video) {
    ...
    if (video.querySelector('a').classList.contains('add-list')) {
        ...
        searchVideoOne = searchVideoOne.filter(video => video.id !== itemVideo.id)
    ...
    if (video.querySelector('a').classList.contains('remove-list')) {
        ...
        //Delete playList
        searchVideoTwo = searchVideoTwo.filter(video => video.id !== itemVideo.id)
...
const removeSearchWordAction = (e) => {
    ...
    searchVideoWord.forEach(word => {
        ...
        searchVideoOne = searchVideoOne.filter(video => video.titulo.includes(word) || video.artista.includes(word))
        searchVideoTwo = searchVideoTwo.filter(video => video.titulo.includes(word) || video.artista.includes(word))
    ...
    searchVideoWord.length === 0 ? cleanToSearchButton.setAttribute('hidden', '') : null
